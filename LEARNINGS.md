...
- **[pattern:project]** Dependencies allowed to run install scripts (`@vscode/vsce-sign`, `keytar` for `semantic-release-vsce`, `sharp`) are listed under `allowBuilds` in `pnpm-workspace.yaml`. pnpm 11 ignores the `pnpm` field of `package.json` and warns on every command when it is present.
