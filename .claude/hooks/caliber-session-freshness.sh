...
if [ "$COMMITS_BEHIND" -gt 15 ]; then
  printf '{"systemMessage":"Caliber: agent configs are %s commits behind. Run /Users/normandy/.nvm/versions/node/v26.2.0/bin/caliber refresh to sync."}' "$COMMITS_BEHIND"
fi
