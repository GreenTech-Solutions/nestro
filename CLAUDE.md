...
- `metadataRegistry.ts` — resolves package-manager-aware metadata adapters in tier order (native CLI, config-aware HTTPS, public npm); the current registry contains the config-aware HTTPS fallback for all managers
- `registryClient.ts` — implements the config-aware HTTPS metadata adapter; resolves the registry URL from `.npmrc` (user home then workspace root, including scoped `@scope:registry` entries), parses versions, dist-tags, and optional publish times, and returns bounded typed outcomes
