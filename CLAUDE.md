...
- `metadataRegistry.ts` — resolves package-manager-aware metadata adapters in tier order (native CLI, config-aware HTTPS, public npm); the current registry uses native CLI for npm/pnpm and the config-aware HTTPS fallback for every manager
...
- `registryClient.ts` — implements the config-aware HTTPS metadata adapter; resolves nested project-to-user `.npmrc` settings and `npm_config_*` overrides for scoped registries, path-scoped auth, TLS, proxies, and bounded redirects, then parses versions, dist-tags, and optional publish times
